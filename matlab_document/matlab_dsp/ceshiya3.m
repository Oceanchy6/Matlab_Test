...
for snr=-30:0.1:10        %401个信噪比SNR
    for  j=1:400            %做400次估计
      ...
      Xk=abs(fft(yn,N));
      for k=1:N/2
         half_Xk(k)=Xk(k);               %取幅频响应前N/2个点
      end    
    [pks,locs]=findpeaks(half_Xk,'SortStr','descend');%寻峰并降序排列
    f1_shuzu(j)=locs(1)/N;                      %将估计的频率正整数值转换为真实值
    f2_shuzu(j)=locs(2)/N; 
    end  
    f1_aver(i)=mean(f1_shuzu);  %对400次估计的f1向量取平均值
    f2_aver(i)=mean(f2_shuzu);  %对400次估计得f2向量取平均值
    f1_mse(i)=var(f1_shuzu)+(f1-f1_aver(i)).^2; %求该SNR下所得的频率估计的均方误差
